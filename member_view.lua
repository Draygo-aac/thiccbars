...
function IsInWatchList(buffInfo, dead)
  ...
  if dead then
    ...
    return DICTONARY_WATCH_DEBUFF_ID[buffInfo.buff_id], buffInfo.buff_id == 2167, DICTIONARY_WATCH_DISABLES_ID[buffInfo.buff_id]
  end
...
function GetWatchedDebuffs(target, dead)
  ...
  for i = 1, count do
    ...
    local watched, pvp, disable = IsInWatchList(buffInfo, dead)
    if watched or disable then
...
function CreateBuffWindow(id, window, maxCount)
  ...
  function w:BuffUpdate(target, dead)
    ...
    local count
    local debuffs, pvpDebuff, disabled = GetWatchedDebuffs(target, dead)
...
function SetViewOfRaidMember(name, ownId, index, parent)
  ...
  hpBar:ApplyBarTexture(HP_STYLE)  --HP_RAID STATUSBAR_STYLE.HP_RAID_OFFLINE

  hpBar.statusBar:SetBarColor(TEAM_ROLE_COLORS.undecided[1]/255, TEAM_ROLE_COLORS.undecided[2]/255, TEAM_ROLE_COLORS.undecided[3]/255, TEAM_ROLE_COLORS.undecided[4]/255)
  w.hpBar = hpBar
